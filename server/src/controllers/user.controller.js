...
const { storeGoogleUser, insertNewUser, fetchUserByEmail } = require("../models/users.model");
...
async function controllerAuthGoogle(req, res) {
	try {
		...
		let token = jwt.sign({ _id: response._id, email: response.email }, process.env.JWT_SECRET, { expiresIn: "2h" });
...
async function controllerVerifyUserCredentials(req, res) {
	try {
		...
		if (!exists) {
			res.status(404).json({ error: "User not found. Please check credentials and try again." });
			return;
		}
		console.log(exists);

		if (exists.provider !== "local") {
			res.status(401).json({ error: "Account is associated with google please sign in with google" });
			return;
		}
		...
		if (!valid) {
			res.status(404).json({ error: "Invalid user credentials. Please check credentials and try again." });
			return;
		}

		// Create a token
		let token = await jwt.sign({ _id: exists._id, email: exists.email }, process.env.JWT_SECRET, { expiresIn: "30d" });
...
module.exports = { controllerAuthGoogle, controllerInsertUser, controllerVerifyUserCredentials, controllerFetchLoggedInUser };
